...
if(!empty($_POST))
{
	...
	if($name!="")
	{
		$newFunctionality = new Functionality($bdd);

		if($newFunctionality->existFunctionality($name)==1)
		{
			$error = "Your functionality already exist";
			$class_message = "error";
		}
		else
		{
			$newFunctionality->addFunctionality($name);

			$error = "Your functionality has been created";
			$class_message = "validate";
		}
	}
	else
	{
		$error = "You must add a functionality name";
		$class_message = "error";		
	}
...
<body>
	...
	<main>
		...
		<div class="container admin">
			...
			?>	
			<br/><br/>
			<form action="add_functionality.php" method="POST">
				<label for="label">Name:</label>
				<input type="text" name="name" id="label" value="<?php echo $name ?>" />
				<br/>
				<br/>
				<input type="submit" value="Add functionality">
			</form>
