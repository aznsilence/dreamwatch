...
if(!empty($_POST))
{
	...
	if($name!="")
	{
		$newShape = new Shape($bdd);

		if($newShape->existShape($name)==1)
		{
			$error = "Your shape already exist";
			$class_message = "error";
		}
		else
		{
			$newShape->addShape($name);

			$error = "Your shape has been created";
			$class_message = "validate";
		}
	}
	else
	{
		$error = "You must add a shape name";
		$class_message = "error";		
	}
...
<body>
	...
	<main>
		...
		<div class="container admin">
			...
			?>	
			<br/><br/>
			<form action="add_shape.php" method="POST">
